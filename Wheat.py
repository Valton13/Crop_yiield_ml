# wheat_yield_forecast.py
# Predict Wheat Yield (kg/ha) for 2019, 2020, 2021 for Each District in Punjab
...
print("🌾 Wheat Yield Prediction for 2019–2021 (Punjab, India)")

# --- Step 1: Load Data ---
df = pd.read_csv('Table_4.7_Yield_Wheat_1 (1).csv')
...
pred_df.to_csv('Wheat_Yield_Predictions_2019_2021.csv')

print(f"\n📊 Predictions saved to 'Wheat_Yield_Predictions_2019_2021.csv'")
...
if sample_district in pred_df.columns:
    ...
    plt.title(f"Wheat Yield Trend & Forecast: {sample_district}")
    plt.xlabel("Year")
